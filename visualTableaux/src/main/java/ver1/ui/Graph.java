...
//		gn.paint(g);
		...
		System.out.println(rootInfo.getLeaves());
		System.out.println(workspaceLength);
		...
		System.out.println(CANVAS_WIDTH/2-workspaceLength/2 + ", " + (CANVAS_WIDTH/2+workspaceLength/2));
		rootInfo.setTopRight(new Point(CANVAS_WIDTH/2-GraphNode.getWidth()/2, 100));
		System.out.println(CANVAS_WIDTH/2-GraphNode.getWidth()/2);
		// Adjust canvas size
		...
		while (!q.isEmpty()) {
			...
			// Draw GraphEdge if there's a parent
			if (gnIH.getParentBottomLeft() != null) {
				// TODO
			}
			...
			String[] axioms = (sAxioms.substring(1, sAxioms.length()-1)).split(", ");
			GraphNode gn = new GraphNode(tn.getData().getId(), axioms, gnIH.getTopRight().x, gnIH.getTopRight().y);
			...
			for (TreeNode child : tn.getChildren()) {
				...
				ih.setParentBottomLeft(new Point(gnIH.getTopRight().x, gnIH.getTopRight().y+gn.getHeight()));
